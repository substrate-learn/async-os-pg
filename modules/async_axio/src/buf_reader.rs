...
use crate::{AsyncBufRead, AsyncRead, AsyncSeek, SeekFrom, IoSliceMut, Read, DEFAULT_BUF_SIZE};
use crate::Result;
...
impl<R: AsyncRead> AsyncBufReader<R> {
    ...
    /// # Ok(()) }) }
    /// ```
    pub const fn new(inner: R) -> AsyncBufReader<R> {
...
impl<R: AsyncRead + fmt::Debug> fmt::Debug for AsyncBufReader<R> {
...
impl<R: AsyncSeek> AsyncSeek for AsyncBufReader<R> {
