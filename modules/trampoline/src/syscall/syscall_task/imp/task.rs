...
use sync::Mutex;
use taskctx::{BaseScheduler, TaskId};
// use sync::Mutex;
...
pub async fn syscall_exec(args: [usize; 6]) -> SyscallResult {
    ...
    if crate::exec(curr_process.as_executor_ref(), path, args_vec, &envs_vec).await.is_err() {
        // exit_current_task(0);
...
pub async fn syscall_clone(args: [usize; 6]) -> SyscallResult {
    ...
    if let Ok(new_task_id) = crate::clone_task(
        curr_process.as_executor_ref(), flags, stack, ptid, tls, ctid, sig_child).await {
...
pub async fn syscall_wait4(args: [usize; 6]) -> SyscallResult {
    ...
    loop {
        ...
        log::debug!("wait_pid answer {:?}", answer);
        match answer {
            Ok(pid) => {
...
pub async fn syscall_prctl(args: [usize; 6]) -> SyscallResult {
    use crate::syscall::{PrctlOption, PR_NAME_SIZE};
